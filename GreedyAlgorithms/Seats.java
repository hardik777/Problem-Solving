...
public class Seats {
    ...
    public int seats(String A) {
        ...
        int mid_index = (int) (pos.size() - 1) >> 1;

        for (int i = mid_index + 1; i < pos.size(); i++) {
            if (pos.get(i) - pos.get(i - 1) > 1) {
                ans = (ans + pos.get(i) - pos.get(i - 1) - 1) % MOD;
                pos.set(i, pos.get(i - 1) + 1);
            }
        }

        for (int i = mid_index - 1; i >= 0; i--) {
            if (pos.get(i + 1) - pos.get(i) > 1) {
                ans = (ans + pos.get(i + 1) - pos.get(i) - 1) % MOD;
                pos.set(i, pos.get(i + 1) - 1);
            }
        }
