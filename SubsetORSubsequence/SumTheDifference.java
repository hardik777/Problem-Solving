...
public class SumTheDifference {
    ...
    public static void main(String[] args) {
        ArrayList<Integer> arr = new ArrayList<Integer>(Arrays.asList(3, 5, 10));
        int solve = solve(arr);
    ...
    public static int findMaxSum(ArrayList<Integer> A) {
        //find max sum
        int ans = 0;
        for (int i = 0; i < A.size(); i++) {
            ans += A.get(i) * Math.pow(2, i) % mod;
            ans = ans % mod;
        }
        return ans;
    }

    public static int findMinSum(ArrayList<Integer> A) {
        //find min sum
        int ans = 0;
        int j = 0;
        for (int i = A.size() - 1; i >= 0; i--) {
            ans += A.get(i) * Math.pow(2, j) % mod;
            ans = ans % mod;
            j++;
        }
        return ans;
